...
export default function BillingPage({ toast }: Props) {
  ...
  // Billing entitlement is server-owned and Stripe-verified. Use the secure
  // billing-summary response as the source of truth once it is available;
  // profile is only a pre-load fallback so this page cannot remain stuck on
  // a stale client-side Free value after a successful Plus activation.
  const verifiedEntitlement = billingDetails?.entitlement
  const isPlus = verifiedEntitlement ? verifiedEntitlement.plan === 'plus' : profile?.plan === 'plus'
  const isOwner = profile?.role === 'owner'
  const billingInterval = verifiedEntitlement?.billingInterval
    ?? ((profile as any)?.billingInterval as 'monthly' | 'annual' | undefined)
    ...
    const loadBillingDetails = async (showLoading = false) => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setDetailsError(null)
          // Keep the broader auth profile in sync for the rest of the app, but
          // Billing itself renders from the verified response above.
  ...
  return (
    <div style={{ padding: 32, maxWidth: 800 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 32 }}>
        <div style={{ background: '#fff', border: '2px solid var(--border)', borderRadius: 16, padding: 20 }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 20px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {FREE_FEATURES.map(f => <li key={f} style={{ fontSize: 12, color: 'var(--dark)', display: 'flex', gap: 7 }}><span style={{ color: 'var(--mid)', flexShrink: 0 }}>✓</span>{f}</li>)}
          </ul>
          <div style={{ textAlign: 'center', padding: '9px', background: !isPlus ? 'var(--green-light)' : 'var(--sand)', borderRadius: 10, fontSize: 12, fontWeight: 600, color: !isPlus ? 'var(--green)' : 'var(--mid)' }}>
      ...
      <div style={{ background: 'var(--sand)', borderRadius: 12, padding: '14px 20px', marginBottom: 24, fontSize: 13, color: 'var(--mid)' }}>
        🐾 <strong>1-2 dogs?</strong> iDogs is free forever for up to 2 dogs — no credit card, no expiry. Ownership transfer and your dog's permanent QR Passport are free on every plan.
      </div>
      ...
      <div className="card" style={{ marginBottom: 24, padding: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
          ...
          <div style={{ minWidth: 220, flex: '0 1 260px' }}>
            {detailsLoading ? <div style={{ fontSize: 13, color: 'var(--light)' }}>Loading SMS status…</div> : detailsError ? <div role="alert" style={{ fontSize: 13, color: 'var(--danger, #C0392B)' }}>SMS status temporarily unavailable.</div> : (
              <>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--dark)', marginBottom: 8 }}>Status: <span style={{ textTransform: 'capitalize' }}>{billingDetails?.sms.status || 'inactive'}</span></div>
                {billingDetails?.sms.status === 'active' && (
                  <>
                    <div style={{ fontSize: 13, color: 'var(--mid)', marginBottom: 6 }}>Usage: <strong>{billingDetails.sms.creditsUsed} / {billingDetails.sms.creditsLimit}</strong> credits</div>
                    <div style={{ height: 8, borderRadius: 999, background: 'var(--sand)', overflow: 'hidden', marginBottom: 8 }}><div style={{ width: `${Math.min(100, (billingDetails.sms.creditsUsed / Math.max(1, billingDetails.sms.creditsLimit)) * 100)}%`, height: '100%', background: 'var(--green)' }} /></div>
                    {billingDetails.sms.creditsUsed >= billingDetails.sms.creditsLimit && <div role="alert" style={{ fontSize: 12, color: 'var(--danger, #C0392B)', marginBottom: 8 }}>SMS credits exhausted. Email and in-app reminders continue normally.</div>}
                    {billingDetails.sms.creditsUsed < billingDetails.sms.creditsLimit && billingDetails.sms.creditsUsed >= billingDetails.sms.creditsLimit * 0.8 && <div style={{ fontSize: 12, color: 'var(--gold)', marginBottom: 8 }}>You have used at least 80% of this month's SMS credits.</div>}
                  </>
                )}
                {billingDetails?.sms.status === 'active' || billingDetails?.sms.status === 'past_due' ? (
                  ...
                  <button className="btn btn-secondary" type="button" disabled title="Stripe SMS add-on is not configured for this environment">SMS add-on coming soon</button>
                ) : (
                  <button className="btn btn-primary" type="button" onClick={handleSmsSubscribe} disabled={smsCheckoutLoading}>{smsCheckoutLoading ? 'Adding SMS…' : 'Add SMS — $3/month'}</button>
                )}
              </>
            )}
          </div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        ...
        {detailsLoading ? <div style={{ fontSize: 13, color: 'var(--light)' }}>Loading payment history…</div> : detailsError ? <div role="alert" style={{ fontSize: 13, color: 'var(--danger, #C0392B)' }}>{detailsError}</div> : !billingDetails?.invoices.length ? <div style={{ fontSize: 13, color: 'var(--light)' }}>No invoices yet.</div> : (
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {billingDetails.invoices.map((invoice, index) => (
              <div key={invoice.id} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 12, alignItems: 'center', padding: '12px 0', borderTop: index ? '1px solid var(--sand)' : 'none', fontSize: 12 }}>
                <div><div style={{ fontWeight: 600, color: 'var(--dark)' }}>{invoice.number || 'Stripe invoice'}</div><div style={{ color: 'var(--light)', marginTop: 2 }}>{formatBillingDate(invoice.createdAt)}</div></div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>{invoice.hostedInvoiceUrl && <a className="btn btn-secondary btn-sm" href={invoice.hostedInvoiceUrl} target="_blank" rel="noreferrer">View</a>}{invoice.invoicePdf && <a className="btn btn-secondary btn-sm" href={invoice.invoicePdf} target="_blank" rel="noreferrer">PDF</a>}</div>
              </div>
            ))}
          </div>
        )}
