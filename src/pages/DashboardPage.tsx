...
export default function DashboardPage({ toast }: Props) {
  ...
  const STATS = [
    ...
    { value: overdueCount,                                          label: 'Overdue reminders', icon: '🔔',  link: '/app/reminders', color: overdueCount > 0 ? 'var(--danger)' : undefined },
    { value: activeDogs.filter(d => d.lifeStage === 'puppy').length, label: 'Puppies',          icon: '🐾',  link: '/app/dogs',      color: undefined },
