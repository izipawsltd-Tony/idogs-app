...
export default function SuperAdminOrganisationsPage() {
  ...
  const freeOrgs = organisations.filter(o => o.plan === 'free').length
  const plusOrgs = organisations.filter(o => o.plan === 'plus').length
  const activePaidOrgs = organisations.filter(o => o.plan === 'plus' && ['active', 'past_due'].includes(o.status.toLowerCase())).length
    ...
    <div className="super-admin-page">
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
        ...
        <div className="super-admin-module-card">
          ...
          <h3 style={{ fontSize: 28, margin: '6px 0 0', fontWeight: 700, color: '#c8971f' }}>{freeOrgs}</h3>
        </div>
        <div className="super-admin-module-card">
          <span>PLUS ENTITLED</span>
          <h3 style={{ fontSize: 28, margin: '6px 0 0', fontWeight: 700, color: '#085041' }}>{plusOrgs}</h3>
        </div>
        <div className="super-admin-module-card">
          <span>ACTIVE PAID</span>
          <h3 style={{ fontSize: 28, margin: '6px 0 0', fontWeight: 700, color: '#1a3a2a' }}>{activePaidOrgs}</h3>
        </div>
      </section>
                  ...
                  <tr key={org.id} style={{ borderBottom: '1px solid #f4f6f5' }}>
                    ...
                    <td style={{ padding: '10px 8px' }}>
                      <span style={{
                        ...
                        padding: '2px 6px',
                        borderRadius: 4,
                        background: org.plan === 'free' ? '#e2e8f0' : '#e1f5ee',
                        color: org.plan === 'free' ? '#475569' : '#085041',
