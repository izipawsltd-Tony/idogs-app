...
test('server-confirmed sent message appears once and deduplicates by ID',()=>assert.ok(widget.includes('id: result.messageId')&&widget.includes('new Map(current.map(item => [item.id, item]))')))
test('active user thread survives list polling',()=>assert.ok(widget.includes('if (activeRef.current) await refreshConversation(activeRef.current)')&&widget.includes('activeRef.current !== id')))
