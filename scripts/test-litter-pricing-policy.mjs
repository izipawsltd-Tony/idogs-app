...
check('Plus monthly policy is A$7', PLUS_MONTHLY_PRICE_AUD === 7)
check('Plus annual policy is A$70', PLUS_ANNUAL_PRICE_AUD === 70)
check('client Plus litter allowance is 2', LITTER_QUOTA_PLUS_ROLLING_12_MONTHS === 2)
check('server Plus litter allowance is 2', LITTER_INCLUDED_QUOTA === 2)
check('client/server Extra Litter price is A$39', EXTRA_LITTER_PRICE_AUD === 39 && SERVER_EXTRA_LITTER_PRICE_AUD === 39 && EXTRA_LITTER_PRICE_CENTS === 3900)
...
check('Terms describe one-time A$39 Extra Litter credit', terms.includes('Extra Litter Credit — A${EXTRA_LITTER_PRICE_AUD} each'))
