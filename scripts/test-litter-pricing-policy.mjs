...
check('quota loading state is explicit while create remains fail-closed', cta.includes('Checking litter access…') && cta.includes("setQuotaLoadState('loading')"))
check('quota load failure is explicit while create remains fail-closed', cta.includes('Litter access unavailable — refresh to try again') && cta.includes("setQuotaLoadState('error')"))
check('quota status copy is announced accessibly', cta.includes('aria-live="polite"'))
