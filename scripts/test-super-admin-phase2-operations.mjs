...
await test('audit rows expose accessible responsive expand/collapse details', () => {
  ...
  assert.match(source, /technicalLogId/)
  assert.match(source, /Show technical data/)
  assert.match(source, /Hide technical data/)
  assert.match(source, /aria-expanded=\{isTechnicalExpanded\}/)
  assert.match(source, /aria-controls=\{technicalPanelId\}/)
  assert.match(source, /isTechnicalExpanded && \(/)
  assert.match(source, /setTechnicalLogId\(null\)/)
  assert.doesNotMatch(source, /<span>\{auditStateSummary\(l\.beforeState\)\}<\/span><pre>/)
  assert.doesNotMatch(source, /<span>\{auditStateSummary\(l\.afterState\)\}<\/span><pre>/)
  ...
  assert.match(css, /\.super-admin-audit-technical-grid[\s\S]*grid-template-columns: repeat\(2/)
  assert.match(css, /@media \(max-width: 560px\)[\s\S]*\.super-admin-audit-technical-grid[\s\S]*grid-template-columns: 1fr/)
})

await test('audit summaries omit expiry text for revoked entitlements', () => {
  const source = fs.readFileSync(path.join(root, 'src/super-admin/pages/SuperAdminAuditLogsPage.tsx'), 'utf8')
  assert.match(source, /entitlement\.granted \? `granted\$\{entitlement\.expiresAt/)
