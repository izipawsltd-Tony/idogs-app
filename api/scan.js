...
import { createScanHandler } from './_lib/scan-handler.js'
import { reserveScanQuota, rollbackScanReservation } from './_lib/scan-quota.js'
...
async function callAnthropic({ image, mediaType }) {
  const apiKey = process.env.ANTHROPIC_API_KEY
  if (!apiKey) {
    return { ok: false, status: 500, details: { error: 'ANTHROPIC_API_KEY not set' } }
  }
  ...
  try {
    ...
    return { ok: true, extracted: { documentType: 'other', vaccines: [], healthTests: [], raw: text } }
  }
}
...
export default createScanHandler({
  ...
  reserveQuota: userRef => reserveScanQuota(db, userRef),
  rollbackQuota: (userRef, plan, periodStart) => rollbackScanReservation(db, userRef, plan, periodStart),
