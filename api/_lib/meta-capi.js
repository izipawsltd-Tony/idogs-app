...
//
// Acquisition policy:
//   subscription_create + Plus           -> Purchase
//   subscription_create + Plus + SMS     -> ONE Purchase, actual invoice amount
//   subscription_update + positive SMS   -> SmsAddonPurchase (custom event)
//   subscription_cycle / renewals        -> no acquisition event
//
// Value always comes from Stripe invoice.amount_paid, never a browser value
// or a hard-coded plan price. A stable Meta event_id plus a durable Firestore
// delivery record makes retries effectively-once for Meta counting even if a
// network failure happens after Meta accepted an HTTP request.

import { createHash } from 'node:crypto'
import { CHECKOUT_PRICE_IDS } from './checkout-handler.js'

export const META_DATASET_ID = '2647777915638218'
...
export function buildMetaServerEvent({ stripeEvent, invoice, subscription, classification }) {
  ...
  return {
    ...
    event_source_url: 'https://idogs.com.au/',
    user_data: {
