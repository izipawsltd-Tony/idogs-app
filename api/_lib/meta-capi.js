...
function resolvedTestEventCode() {
  if (process.env.META_CAPI_TEST_EVENT_CODE) return process.env.META_CAPI_TEST_EVENT_CODE
  const isStagingPreview = process.env.FIREBASE_PROJECT_ID === 'idogs-app-staging' && process.env.VERCEL_ENV === 'preview'
  return isStagingPreview ? 'TEST97167' : undefined
}
